...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot DDPG training curves from metrics.csv.")
    parser.add_argument("--log-file", type=Path, default=Path("logs_ddpg/metrics.csv"))
    parser.add_argument("--output", type=Path, default=Path("logs_ddpg/training_curves.png"))
...
def main() -> None:
    ...
    plot_group(axes[1], x_values, metrics, ["success_rate", "episode_collision_rate", "collision_rate"], args.smooth)
    ...
    plot_group(axes[3], x_values, metrics, ["actor_loss", "critic_loss"], args.smooth)
    axes[3].set_title("Losses")
    axes[3].set_xlabel("steps")

    plot_group(axes[4], x_values, metrics, ["noise_std", "replay_size"], args.smooth)
    axes[4].set_title("DDPG Diagnostics")
